...
let handle_push from_left key values ({ store; _ } as state) =
  let l =
  ...
  | Some l ->
    let l = if from_left then List.append (List.rev values) l else List.append l values in
    let store = StringMap.add key (LIST l) store in
    Response.INTEGER (List.length l), { state with store }
;;
